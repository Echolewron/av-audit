...
const sermonSenderView = {
  ...
  bindEvents() {
    ...
    // Toggle Webhook Body field visibility based on Method (GET vs POST)
    const selectWebhookMethod = document.getElementById('sermon-cfg-webhook-method');
    const groupWebhookBody = document.getElementById('sermon-cfg-webhook-body-group');
    if (selectWebhookMethod && groupWebhookBody) {
      selectWebhookMethod.addEventListener('change', () => {
        groupWebhookBody.style.display = selectWebhookMethod.value === 'GET' ? 'none' : 'block';
      });
    }
  ...
  openSettingsModal() {
    ...
    const inputSenderName = document.getElementById('sermon-cfg-sender-name');
    const inputWebhookMethod = document.getElementById('sermon-cfg-webhook-method');
    const inputWebhookUrl = document.getElementById('sermon-cfg-webhook-url');
    const inputWebhookBody = document.getElementById('sermon-cfg-webhook-body');
    const groupWebhookBody = document.getElementById('sermon-cfg-webhook-body-group');
    ...
    if (inputSenderName) inputSenderName.value = this.settings.sender_name || '';
    if (inputWebhookMethod) inputWebhookMethod.value = this.settings.webhook_method || 'POST';
    if (inputWebhookUrl) inputWebhookUrl.value = this.settings.webhook_url || '';
    if (inputWebhookBody) inputWebhookBody.value = this.settings.webhook_body || '';
    if (groupWebhookBody && inputWebhookMethod) {
      groupWebhookBody.style.display = inputWebhookMethod.value === 'GET' ? 'none' : 'block';
    }

    // If webhook_url or custom sender_name is configured, open the advanced details accordion by default
    const detailsAdvanced = document.getElementById('sermon-advanced-details');
    if (detailsAdvanced) {
      if (this.settings.webhook_url || (this.settings.sender_name && this.settings.sender_name !== 'AV Audit Recordings Sender')) {
        detailsAdvanced.open = true;
      }
    }
  ...
  async handleSaveSettings() {
    ...
    const payload = {
      ...
      sender_name: inputSenderName ? inputSenderName.value.trim() : '',
      webhook_method: inputWebhookMethod ? inputWebhookMethod.value : 'POST',
      webhook_url: inputWebhookUrl ? inputWebhookUrl.value.trim() : '',
      webhook_body: inputWebhookBody ? inputWebhookBody.value : '',
